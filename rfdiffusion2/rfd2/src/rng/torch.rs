...
pub struct Mt19937 {
    ...
    left: i32,
    next: usize,
    /// Number of 32-bit draws taken since construction. Diagnostic only: when a
    /// forward pass disagrees with the reference *after* the first block, the
    /// first thing to rule out is a stream-position drift, and a count is much
    /// cheaper to compare than a state dump.
    draws: u64,
}
...
impl Mt19937 {
    pub fn new(seed: u64) -> Self {
        ...
        Mt19937 { state, left: 1, next: 0, draws: 0 }
    }

    /// Rebuild the engine from `torch.get_rng_state()`'s raw bytes.
    ///
    /// Used to start a Rust forward pass at exactly the stream position the
    /// reference was at — which the network needs, because RFdiffusion2 runs
    /// **with dropout active at inference** (see `crate::dropout`) and therefore
    /// consumes the generator inside the forward pass.
    ///
    /// Layout, decoded from a live 2.4.0 state rather than from the header:
    /// `u64 seed | i32 left | i32 seeded | u64 next | u64 state[624] | …`.
    /// The 624 state words are stored **one per u64**, not packed — reading them
    /// as `u32` finds nothing and silently yields a zeroed engine.
    pub fn from_torch_state(bytes: &[u8]) -> Self {
        assert!(bytes.len() >= 24 + MT_N * 8, "rng state too short: {}", bytes.len());
        ...
        let left = rd_i32(8);
        let next = rd_u64(16) as usize;
        let mut state = [0u32; MT_N];
        for (j, s) in state.iter_mut().enumerate() {
            *s = rd_u64(24 + j * 8) as u32;
        }
        Mt19937 { state, left, next, draws: 0 }
    }
    ...
    /// How many 32-bit draws have been taken.
