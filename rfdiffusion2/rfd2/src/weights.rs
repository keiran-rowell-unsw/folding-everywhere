...
impl Weights {
    ...
    /// Fetch a tensor as `f64` **without narrowing**.
    ///
    /// Required by rung 2: the numpy/SciPy side of RFdiffusion2's randomness is
    /// computed in double (`Rotation.random` is f64 throughout and only
    /// `_uniform_so3` casts down), and rung 2's tolerance is exactly 0 — so the
    /// comparison has to happen on the f64 bits, not on an f32 shadow of them.
    pub fn get_f64(&self, name: &str) -> (Vec<f64>, Vec<usize>) {
        let e = self
            .index
            .get(name)
            .unwrap_or_else(|| panic!("weight not found: {name}"));
        let bytes = &self.backing.bytes()[e.start..e.end];
        let data: Vec<f64> = match e.dtype.as_str() {
            "F64" => bytes
                .chunks_exact(8)
            ...
            "F32" => bytes
                ...
                .collect(),
            other => panic!("unsupported float dtype {other} for {name}"),
        };
        (data, e.shape.clone())
    }

    /// Fetch an unsigned integer tensor as `u64` (`random.getrandbits`).
    pub fn get_u64(&self, name: &str) -> (Vec<u64>, Vec<usize>) {
        let e = self
            .index
            .get(name)
            .unwrap_or_else(|| panic!("weight not found: {name}"));
        let bytes = &self.backing.bytes()[e.start..e.end];
        let data: Vec<u64> = match e.dtype.as_str() {
            "U64" => bytes
                .chunks_exact(8)
            ...
            "I64" => bytes
                ...
                .collect(),
            other => panic!("unsupported uint dtype {other} for {name}"),
        };
        (data, e.shape.clone())
    }
